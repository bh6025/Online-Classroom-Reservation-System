...
public class HomeController {
	...
	public String loginConfirm(HttpServletRequest request, Model model) {
		...
		if(request.getParameter("checkType") == null) {
			...
			if((dao.loginDao(request.getParameter("userID"), request.getParameter("userPW")) == 1)
					&& dao.getUserInfoDao(request.getParameter("userID")).getType().equals("admin")) {
				HttpSession session = request.getSession(true);
				session.setAttribute("id", request.getParameter("userID"));
	...
	public String reservationConfirm(HttpServletRequest request, Model model) {
		...
		if(request.getParameter("reservationTime").equals("9"))
			date = date.substring(0, 10) + " 0" + request.getParameter("reservationTime") + ":00:00";
		else
			date = date.substring(0, 10) + " " + request.getParameter("reservationTime") + ":00:00";
		
		ReservationDao dao = sqlSession.getMapper(ReservationDao.class);
		dao.insertReservation(id, request.getParameter("buildingName"), request.getParameter("classRoomName"), date, request.getParameter("userReason"));
		
		return "forward:timeTable";
	}
	...
	public String rejectReservation(HttpServletRequest request, Model model) throws UnsupportedEncodingException {
		request.setCharacterEncoding("UTF-8");
		
		AdminReservationDao dao = sqlSession.getMapper(AdminReservationDao.class);
		//decodeURIComponent(request.getParameter("adminReason")); 	
		System.out.println(request.getParameter("adminReason"));
		System.out.println(URLDecoder.decode(request.getParameter("adminReason"),"UTF-8"));
		
		dao.rejectReservation(request.getParameter("userID"), request.getParameter("buildingName"), request.getParameter("classRoomName"), request.getParameter("reservationTime"), URLDecoder.decode(request.getParameter("adminReason"),"UTF-8"));
